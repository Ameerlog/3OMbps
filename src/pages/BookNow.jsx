...
//           <h2 className="text-3xl md:text-4xl font-bold text-slate-800 max-w-3xl">
...
//                         ? "bg-[#C4D9FF] cursor-not-allowed"
//                         : "bg-[#C5BAFF] hover:bg-[#b1a0ff]"
//                     }`}
//                   >
//                     Next
//                   </button>
...
const DEVICE_TYPES = [
  { label: "Hard Drive", value: "Hard Drive" },
  { label: "SSD", value: "SSD" },
  { label: "SD Card", value: "SD card" },   // match Zoho option text
...
// NEW ContactUs form endpoint (LHyWprw...)
const ZOHO_FORM_ACTION =
  "https://forms.zohopublic.in/30mbpscomgm1/form/ContactUs/formperma/LHyWprwCbFtlGVtjASARfH_eRwgMJnblMYvvlYFbatY/htmlRecords/submit";
...
function Contact() {
  ...
  const STEPS = showGST
    ? ["Device", "Capacity", "Condition", "GST", "Contact", "Review"]
    : ["Device", "Capacity", "Condition", "Contact", "Review"];
  ...
  const canGoNext = () => {
    ...
    // GST validation step
    if (showGST && step === 4) return !!form.gstNo;

    // Contact step: require 10-digit phone
    if ((!showGST && step === 4) || (showGST && step === 5)) {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      // Hidden iframe to avoid redirect / CORS
      const iframe = document.createElement("iframe");
      iframe.name = "zoho-submit-iframe";
      iframe.style.display = "none";
      document.body.appendChild(iframe);

      // Hidden form that posts to Zoho
      ...
      // Zoho meta fields
      addInput("zf_referrer_name", window.location.href);
        ...
        <div className="flex flex-col lg:flex-row gap-10 lg:items-start">
                ...
                >
                      ...
                      <div className="flex flex-wrap gap-3">
                        {DEVICE_TYPES.map((item) => (
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              handleChange("deviceType", item.value)
                            }
                  ...
                  {step === 4 && showGST && (
                    <>
                      ...
                      />
                      {form.gstNo &&
                        !/^[0-9]{2}[A-Z]{5}[0-9]{4}[A-Z][0-9A-Z]$/.test(
                          form.gstNo
                        ) && (
                    ...
                    <>
                      <div className="space-y-1">
                        ...
                        <p className="text-xs text-slate-500">
                          Check your device details and contact information. You
                          can still go back.
                        </p>
                      </div>
                      ...
                      <p className="text-xs text-slate-400">
                        By submitting, you agree to be contacted during business
                        hours.
                      </p>
                    </>
                  )}
          ...
          <aside className="w-full lg:w-1/2 space-y-6">
            <div className="rounded-2xl bg-white/90 border border-slate-100 shadow-sm p-6 md:p-7 space-y-5">
              ...
              <ol className="mt-4 grid grid-cols-4 gap-4">
                {["Your device", "Media capacity", "Condition", "Contact details"].map(
                  (label, index) => {
                    const idx = index + 1;
                    const active = summaryStepIndex === idx;
                    const completed = summaryStepIndex > idx;
